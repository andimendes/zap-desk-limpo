...
import { User, AlertTriangle, Clock } from 'lucide-react'; // 1. Importamos novos ícones

// Função para a barra de progresso (sem alterações)
const FunilProgressBar = ({ etapas, etapaAtualId }) => { /* ...código da barra sem alterações... */ };

// 2. Função auxiliar para calcular a diferença de dias
...
const NegocioCard = ({ negocio, index, onCardClick, etapasDoFunil }) => {
  // 3. Lógica para calcular os dias parado
  // Usamos 'updated_at' como data da última mudança.
  ...
  return (
    <Draggable draggableId={String(negocio.id)} index={index}>
      {(provided, snapshot) => (
        ...
        >
          {/* Seção Superior: Título e Empresa */}
          <div>
          ...
          {/* Seção do Meio: Responsável e Valor */}
          ...
          <div className="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 border-t dark:border-gray-200/50 pt-3">
            ...
            {/* Alerta de Tarefa: só aparece se 'tem_tarefa_futura' for falso */}
            {!negocio.tem_tarefa_futura && (
              <div className="flex items-center gap-1.5 text-yellow-600 dark:text-yellow-500" title="Nenhuma tarefa futura agendada">
