...
const ActivityComposer = ({ negocioId, onActionSuccess }) => {
  ...
  const [atividadeTipo, setAtividadeTipo] = useState('Tarefa');
  ...
  const getCurrentUserId = async () => {
    ...
    if (error || !user) {
      ...
      // O ideal seria redirecionar para a página de login aqui.
      return null;
    }
    return user.id;
  };
  ...
  const handleSaveNota = async () => {
    if (!notaConteudo.trim()) { /* ... validação ... */ return; }
    
    setIsSaving(true);
    try {
      // --- ALTERAÇÃO AQUI ---
      const userId = await getCurrentUserId();
      if (!userId) { // Se não conseguir pegar o user, para a execução.
        setIsSaving(false);
        return;
      }
      ...
      const { error } = await supabase
        ...
        .insert({
          ...
          user_id: userId, // ADICIONAMOS O ID DO USUÁRIO
        });
      // --- FIM DA ALTERAÇÃO ---
  ...
  const handleSaveAtividade = async () => {
    if (!atividadeDescricao.trim() || !atividadeData || !atividadeHora) { /* ... validação ... */ return; }
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-sm">
        {/* ... todo o seu código JSX existente vai aqui, sem nenhuma alteração ... */}
