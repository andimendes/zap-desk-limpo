...
const GestaoDeEquipaPage = ({}) => {
    ...
    const fetchTeamAndRoles = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('/api/get-team-members', { headers: { 'Authorization': `Bearer ${session.access_token}` } });
            if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || 'Falha ao buscar dados da equipe.'); }
    ...
    // ✅ FUNÇÕES ATUALIZADAS PARA CHAMAR AS NOVAS APIs
    const handleSaveUser = async (userId, updatedData) => {
        ...
        if (!response.ok) {
            ...
            alert(`Erro ao atualizar utilizador: ${error.error}`);
        } else {
            setEditingUser(null);
            fetchTeamAndRoles();
        }
    ...
    const executeDelete = async () => {
        ...
        if (!response.ok) {
            const error = await response.json();
            alert(`Erro ao apagar utilizador: ${error.error}`);
        } else {
            fetchTeamAndRoles();
        }
        setIsDeleting(false);
    ...
    const handleResendInvite = async (member) => {
        ...
        if (!response.ok) {
            const error = await response.json();
            alert(`Erro ao reenviar convite: ${error.error}`);
        } else {
...
const InviteUserModal = ({ roles, onClose, onInviteSent }) => { /* ...código do modal de convite não muda... */ };
