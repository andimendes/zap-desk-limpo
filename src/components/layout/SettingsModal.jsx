...
const SettingsModal = ({ isOpen, onClose, userProfile, userAuth }) => {
    ...
    // Todas as outras funções do seu ficheiro original
    const handlePreferencesChange = (key, value) => { setPreferences(prev => ({ ...prev, [key]: value })); if (key === 'theme') { setTheme(value); } };
    const handlePreferencesSave = async (e) => { e.preventDefault(); /* ... */ };
    const handleProfileSave = async (e) => { e.preventDefault(); /* ... */ };
    const handleSecuritySave = async (e) => { e.preventDefault(); /* ... */ };
            ...
            <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col dark:bg-gray-800 dark:border dark:border-gray-700">
                ...
                <div className="flex flex-grow overflow-hidden">
                    ...
                    <main className="w-3/4 p-6 overflow-y-auto">
                        {activeTab === 'profile' && (
                            <div>
                                {/* Conteúdo completo da aba Perfil que você já tinha */}
                            </div>
                        )}
                        {activeTab === 'system' && ( 
                            <form onSubmit={handlePreferencesSave}>
                                {/* Conteúdo completo da aba Preferências que você já tinha */}
                            </form> 
                        )}
