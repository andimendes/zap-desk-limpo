...
import { Clock, Flag, Building } from 'lucide-react';
...
const ChamadoListItem = ({ chamado, onClick }) => {
    ...
    return (
        <tr onClick={onClick} className="bg-white hover:bg-gray-50 cursor-pointer border-b transition-colors">
            <td className="px-6 py-4 whitespace-nowrap">
                <div className="text-sm font-bold text-gray-900">{chamado.titulo}</div>
                <div className="text-sm text-gray-500 flex items-center mt-1">
                    <Building size={14} className="mr-2" />
                    {chamado.clientes?.nome_fantasia || chamado.clientes?.razao_social}
                </div>
            </td>
            <td className="px-6 py-4 whitespace-nowrap">
                <span className={`px-2.5 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${sla.color}`}>
                    {sla.text}
                </span>
            </td>
            <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold">
                <div className={`flex items-center gap-2 ${prioridadeCores[chamado.prioridade]}`}>
                    <Flag size={16} />
                    {chamado.prioridade}
                </div>
            </td>
            ...
            <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-gray-800">{chamado.atendente?.full_name || 'Não atribuído'}</span>
                </div>
            </td>
