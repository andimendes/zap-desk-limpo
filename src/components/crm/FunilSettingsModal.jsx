...
const FunilSettingsModal = ({ isOpen, onClose, funis: initialFunis, onSave }) => {
  ...
  // Sincroniza o estado interno quando o modal abre com novos dados
  useEffect(() => {
    if (initialFunis) {
      setFunis(JSON.parse(JSON.stringify(initialFunis))); // Cria uma cópia profunda para edição segura
    }
  }, [initialFunis, isOpen]);
