...
// ... (Os componentes ConfirmationModal e EditUserModal não mudam e continuam aqui)
...
const EditUserModal = ({ user, allRoles, onClose, onSave, isSaving }) => {
  ...
  const handleSubmit = (e) => { e.preventDefault(); const dataToSave = { name, email, role: selectedRole }; if (showPassword && password) { dataToSave.password = password; } onSave(user.id, dataToSave); };
  if (!user) return null;
...
const GestaoDeEquipaPage = () => {
    ...
    // ✅ FUNÇÃO ATUALIZADA PARA USAR A NOVA API DA VERCEL
    const fetchTeamAndRoles = useCallback(async () => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Falha ao buscar dados da equipe.');
            }
    ...
    const handleSaveUser = async (userId, updatedData) => {
        setIsSaving(true);
        // Esta função provavelmente também deveria ser migrada para a Vercel, mas mantemos por agora.
    ...
    const handleResendInvite = async (member) => {
        // Esta função também pode ser migrada para a Vercel no futuro.
    ...
    return (
        <>
            ...
            <ConfirmationModal isOpen={!!userToDelete} onClose={() => setUserToDelete(null)} onConfirm={executeDelete} isDeleting={isDeleting} title={`Apagar Utilizador "${userToDelete?.name}"`}>Tem a certeza de que quer apagar este utilizador? Esta ação é irreversível e removerá permanentemente o seu acesso.</ConfirmationModal>
        </>
    );
};
const InviteUserModal = ({ roles, onClose, onInviteSent }) => {
    const [email, setEmail] = useState(''); const [fullName, setFullName] = useState(''); const [selectedRole, setSelectedRole] = useState(roles[0]?.name || ''); const [isSending, setIsSending] = useState(false); const [feedback, setFeedback] = useState({ type: '', message: '' });
