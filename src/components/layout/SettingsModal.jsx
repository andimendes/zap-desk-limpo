...
const SettingsModal = ({ isOpen, onClose, userProfile, userAuth }) => {
    ...
    useEffect(() => {
        if (isOpen && userAuth?.id) {
            const fetchInitialData = async () => {
                const { data: prefsData } = await supabase.from('user_preferences').select('*').eq('user_id', userAuth.id).single();
                if (prefsData) { setPreferences(prefsData); setTheme(prefsData.theme); }
                const { data: { session } } = await supabase.auth.getSession();
                const googleId = session?.user?.identities?.find(id => id.provider === 'google');
                if (googleId) { setIsGoogleConnected(true); setGoogleIdentity(googleId); } else { setIsGoogleConnected(false); setGoogleIdentity(null); }
            };
            fetchInitialData();
    ...
    }, [isOpen, userAuth?.id, userProfile, setTheme]);
    ...
    const handleGoogleDisconnect = async () => { if (!googleIdentity) return; if (window.confirm('Tem a certeza que quer desconectar a sua conta Google?')) { const { error } = await supabase.auth.unlinkIdentity(googleIdentity); if (error) { alert('Erro ao desconectar a conta Google.'); } else { setIsGoogleConnected(false); setGoogleIdentity(null); alert('Conta Google desconectada.'); } } };
    ...
    const handleAvatarUpload = async (event) => { try { if (!userAuth?.id) throw new Error("Utilizador não autenticado."); const file = event.target.files[0]; if (!file) return; const fileName = `${userAuth.id}-${Date.now()}`; const filePath = `${fileName}`; const { error: uploadError } = await supabase.storage.from('avatars').upload(filePath, file); if (uploadError) throw uploadError; const { data } = supabase.storage.from('avatars').getPublicUrl(filePath); setAvatarUrl(data.publicUrl); setProfileMessage({ type: 'success', text: 'Avatar pronto. Clique em "Salvar Informações".' }); } catch (error) { setProfileMessage({ type: 'error', text: 'Falha no upload do avatar: ' + error.message }); } };
    ...
    const handleSecuritySave = async (e) => { e.preventDefault(); setIsSavingSecurity(true); setSecurityMessage({ type: '', text: '' }); const { error: reauthError } = await supabase.auth.signInWithPassword({ email: userAuth.email, password: currentPassword }); if (reauthError) { setSecurityMessage({ type: 'error', text: 'Senha atual incorreta.' }); setIsSavingSecurity(false); return; } try { let updates = {}; let successMessage = ''; if (email && email.toLowerCase() !== userAuth.email) { updates.email = email.toLowerCase(); successMessage += 'E-mail atualizado! Verifique a sua caixa de entrada para confirmar. '; } if (newPassword) { if (newPassword.length < 8) throw new Error('A nova senha deve ter no mínimo 8 caracteres.'); if (newPassword !== confirmPassword) throw new Error('As novas senhas não coincidem.'); updates.password = newPassword; successMessage += 'Senha atualizada com sucesso!'; } if (Object.keys(updates).length === 0) { setSecurityMessage({ type: 'info', text: 'Nenhuma alteração preenchida.' }); setIsSavingSecurity(false); return; } const { error } = await supabase.auth.updateUser(updates); if (error) throw error; setSecurityMessage({ type: 'success', text: successMessage.trim() }); clearSecurityFields(); } catch (error) { setSecurityMessage({ type: 'error', text: 'Erro: ' + error.message }); } finally { setIsSavingSecurity(false); setTimeout(() => setSecurityMessage({ type: '', text: '' }), 7000); } };
                        ...
                        <div>
                            ...
                            <form onSubmit={handleProfileSave}>
                                ...
                                <div className="flex items-center gap-6 mb-6">
                                    <UserAvatar name={fullName} src={avatarUrl} /><input type="file" ref={avatarFileRef} onChange={handleAvatarUpload} className="hidden" accept="image/png, image/jpeg" />
                            ...
                            <form onSubmit={handleSecuritySave}>
                                ...
                                <div className="space-y-4">
                                    <InputField icon={<Lock size={16} />} label="Senha Atual" type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} placeholder="••••••••" required/><InputField icon={<Mail size={16} />} label="Alterar E-mail" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="novo@email.com" />
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4"><InputField icon={<Lock size={16} />} label="Nova Senha" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="Mínimo 8 caracteres" /><InputField icon={<Lock size={16} />} label="Confirmar Nova Senha" type="password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repita a nova senha" /></div>
                                </div>
                                <div className="flex justify-between items-center mt-4"><div>{securityMessage.text && (<div className={`flex items-center gap-2 text-sm ${securityMessage.type === 'success' ? 'text-green-600 dark:text-green-400' : securityMessage.type === 'error' ? 'text-red-600 dark:text-red-400' : 'text-blue-600 dark:text-blue-400'}`}>{securityMessage.type === 'success' ? <CheckCircle size={16} /> : <AlertTriangle size={16} />}{securityMessage.text}</div>)}</div><button type="submit" className="px-6 py-2 bg-gray-700 text-white rounded-lg font-semibold transition hover:bg-gray-800 disabled:bg-gray-400 dark:bg-gray-600 dark:hover:bg-gray-500 disabled:cursor-not-allowed" disabled={isSavingSecurity}>{isSavingSecurity ? 'A Atualizar...' : 'Atualizar Segurança'}</button></div>
                            </form>
                        </div>
                        )}
                        {activeTab === 'system' && ( 
                                ...
                                {isGoogleConnected ? (
                                    <div className="flex items-center justify-between p-4 rounded-lg bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800">
                                        <div>
                                            ...
                                            <p className="flex items-center gap-2 text-sm text-green-600 dark:text-green-300"><CheckCircle size={14} /> Conectado com sucesso.</p>
                                        </div>
                                        <button onClick={handleGoogleDisconnect} className="px-4 py-2 bg-red-600 text-white rounded-lg font-semibold transition hover:bg-red-700">Desconectar</button>
                                    </div>
                                ) : (
                                    <div className="flex items-center justify-between p-4 rounded-lg bg-gray-50 dark:bg-gray-700/50">
                                        ...
                                        <button onClick={handleGoogleConnect} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-semibold transition hover:bg-blue-700">Conectar</button>
                                    </div>
                                )}
                            </div>
                        )}
